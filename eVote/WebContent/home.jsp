...
<nav id="menu" style="background:url(image/navBg.png repeat-x;)">
    ...
    <section id="sec">
    	<div id="cont">
    	<h1 style="color:red; font-family:lucida calligraphy; text-decoration:underline;">Candidates of Your Locality:</h1>
    	<%
    	
    	ResultSet rs = ob.fetch(name);
    	
    	String const1 = rs.getString("con");
    	ResultSet res = ob.ucandlist(const1);
    	 %>
    	 ...
    	 <td><%= res.getString("PARTY") %></td>
    	 </tr>
    	 <%} %>
    	 </table>
    	 <br><h1 style="color:red; font-family:lucida calligraphy; text-decoration:underline;">Party Fighting This Year:</h1>
    	 <table class="tab">
    	 <%
    	 	ResultSet ry = ob.party();
    	 ...
    	 while(ry.next()){ %>
    	 <tr>
    	 <td><%= ry.getString("NAME") %></td>
    	 <td><a href="cand.jsp?choice=<%= ry.getString("NAME") %>"><input type="button" value="Candidate List"></a></td>
    	 <td><a href="partyd.jsp?choice=<%= ry.getString("NAME") %>"><input type="button" value="Party Details"></a></td>
    	 </tr>
    	 <%} %>
