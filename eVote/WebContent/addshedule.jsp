...
<nav id="menu" style="background:url(image/navBg.png repeat-x;)">
    ...
    <section id="sec">
    	...
    	<%
    	ResultSet rs= ob.shedule();
    	while(rs.next()){
    	%>
    	<form method="post">
    	<input type="text" name="edate" value="<%=rs.getString("ELECTIONDATE") %>">
    	<input type="text" value="<%=rs.getString("CONSTITUENCY") %>" readonly>
    	<input type="text" name="cdate" value="<%=rs.getString("COUNTINGDATE") %>">
    	<input type="submit" value="Edit" formaction="axx.jsp?name=<%= rs.getString("CONSTITUENCY") %>">
    	</form>
    	<%} %>
    	</div>
